...
# Перетворюємо кортежі у словники
car_dict = {}
...
# Фільтруємо машини за критеріями
filtered_cars = {
    model: info for model, info in car_dict.items()
    if info["year"] >= search_criteria[0]
    and info["engine"] >= search_criteria[1]
    and info["price"] <= search_criteria[2]
}

# Сортуємо за ціною (від найменшої до більшої)
sorted_cars = dict(sorted(filtered_cars.items(), key=lambda x: x[1]["price"]))

# Виводимо максимум 5 машин у порядку: рік, двигун, ціна
count = 0
for model, info in sorted_cars.items():
    print(f"{model}: рік {info['year']}, двигун {info['engine']}, ціна {info['price']}")
    count += 1
    if count == 5:
        break
